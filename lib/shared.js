...
module.exports = {
  ...
  mergeEntries(first, second, ignoreDuplicates) {
    ...
    // a but slower for small ones (~.75x @ ~10).  Pick a cutoff point?
    if (ignoreDuplicates) {
  ...
  normalizeRelativePath(relativePath) {
    ...
    const normalizedPath = path.relative('', path.resolve(relativePath));

    // Paths which are, or start with, `..` escape the root.
    ...
    return normalizedPaths[relativePath] = normalizedPaths[normalizedPath] = normalizedPath;
  },
