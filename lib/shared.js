...
module.exports = {
  ...
  _chompSeparator(path_) {
    const length = path_.length;

    return path_[length - 1] === path.sep ? path_.substring(0, length - 1) : path_;
  },
  ...
  /** Get a path's parent directory.
   *
   * Like `path.dirname`, but uses '' for an empty path rather than '.'.
   *
   * @param {string} path_ The path whose parent should be returned.
   */
  dirname(path_) {
  ...
  sortAndExpand(entries) {
    ...
    for (let i = 0; i < entries.length; ++i) {
      ...
      // Deduplicate in case some wag passes in a list of paths/entries which contains duplicates.
      while ((next = entries[j]) && entry.relativePath === next.relativePath) {
        j++;
      }

      if (i !== j) {
        entries.splice(i + 1, j - i - 1);
      }
   ...
   *
   * For reasons unknown, using String#substring in this manner is ~40x faster
   * than using String#startsWith.
   */
  startsWith: (haystack, needle) => haystack.substring(0, needle.length) === needle,
