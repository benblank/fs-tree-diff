...
Object.assign(ManualTree.prototype, {
  addEntries(entries, options) {
    ...
    this.entries = shared.mergeEntries(this.entries, entries, true);
  },
 ...
 * this type.
 */
class AbstractionError extends TypeError {
 ...
 */
class FacadeTree {
  /** Contruct a new FacadeTree.
   *
   * FacadeTree is an abstract class; this constructor may only be called as
   * `this.super()` from child classes' constructors.
   */
  constructor() {
   ...
   *
   * @param {string} relativePath The path for which to search.  Normalized before use.
   * @private
   * @returns {FacadeTree~FindResult} The results of the search.
   */
  _findByRelativePath(relativePath) {
    return this._findInArrayByRelativePath(this._entries, relativePath);
  }

  /** Find the entry and containing tree, if any, representing the specified path.
   *
   * This is the implementation of _findByRelativePath, genericized so that it
   * be used to search any array (such as the _entriesFromFiles used by some
   * Projections).
   *
   ...
   * @returns {FacadeTree~FindResult} The results of the search.
   */
  _findInArrayByRelativePath(array, relativePath) {
  ...
  _getEntriesFromArrayForDirectory(array, normalizedPath) {
    ...
    if (normalizedPath === '' || array.length && array[0].relativePath.startsWith(normalizedPathWithSeparator)) {
    ...
    while (!entry.relativePath.startsWith(normalizedPathWithSeparator)) {
      index++;
      entry = array[index];
    ...
    while (entry.relativePath.startsWith(normalizedPathWithSeparator)) {
      const relativePath = entry.relativePath.replace(normalizedPathWithSeparator, '');
  ...
  _resolvePath(relativePath) {
    return shared.joinPaths(this.root, shared.normalizeRelativePath(relativePath));
  }
   ...
   */
  chdir(relativePath) {
    const result = this._findByRelativePath(relativePath);
    ...
    return new Projection({
      cwd: shared.normalizeRelativePath(relativePath),
      parent: this,
    });
  }
  ...
  readdirSync(relativePath) {
    ...
    const entries = this._getEntriesForDirectory(shared.normalizeRelativePath(relativePath));
...
Object.assign(Delegator.prototype, {
  ...
  _findByRelativePath(relativePath) {
    return this._delegate._findByRelativePath(relativePath);
  },
...
function filterMatches(normalizedPath, cwd, files, include, exclude) {
  ...
  if (!normalizedPath.startsWith(shared.ensureSeparator(cwd)) || cwd === normalizedPath) {
    return false;
  }
  ...
  if (cwd) {
    normalizedPath = normalizedPath.replace(shared.ensureSeparator(cwd), '');
  }
 ...
 */
class Projection extends FacadeTree {
  /** Create a projection.
   *
  ...
  get entries() {
    ...
    // TODO: Are there still any reasons for this to be a generator rather than
    // pushing to an array?
    function* generateMatchingSubtree(normalizedPath) {
      ...
      for (let i = 0; i < entriesLength; i++) {
        ...
        if (this._filterMatches(entry.relativePath)) {
          ...
          for (let j = 0; j < directoryStackLength; j++) {
            ...
            if (entry.relativePath.startsWith(shared.ensureSeparator(directory.relativePath))) {
              yield directory;
            }
          };
        ...
        if (entry.isDirectory()) {
          const pathWithoutCwd = entry.relativePath.replace(shared.ensureSeparator(this.cwd), '');

          if (mayContain(pathWithoutCwd, this._processedInclude)) {
            yield* generateMatchingSubtree.bind(this)(entry.relativePath);
          }
        }
      }
    }

    return Array.from(generateMatchingSubtree.bind(this)(this._cwd)).map(entry => {
      return Entry.clone(entry, entry.relativePath.replace(shared.ensureSeparator(this._cwd), ''));
  ...
  set files(value) {
    ...
    if (newValue) {
      this._processedFiles = newValue.map(path => shared.normalizeRelativePath(path));
  ...
  _findByRelativePath(relativePath) {
    if (this._entriesFromFiles) {
      return this._findInArrayByRelativePath(this._entriesFromFiles, relativePath);
    }

    // Note that the CWD is included in the normalized path.
    const normalizedPath = shared.joinPaths(this._cwd, shared.normalizeRelativePath(relativePath));

    if (normalizedPath === this._cwd || this._filterMatches(normalizedPath)) {
      return this._parent._findByRelativePath(normalizedPath);
    }
  ...
  _getEntriesForDirectory(normalizedPath) {
    ...
    // FIXME: Needs to filter.
    return this._parent._getEntriesForDirectory(shared.joinPaths(this._cwd, normalizedPath));
  }
  ...
  _shouldExcludeDirectory(normalizedPath) {
    ...
    return shouldExcludeDirectory(normalizedPath.replace(shared.ensureSeparator(this._cwd), ''), exclude);
  }
  ...
  // TODO: Is this even needed?  Why is it the only overridden method?
  existsSync(relativePath) {
    return this._parent.existsSync(shared.joinPaths(this.cwd, relativePath));
  }
}
 ...
 */
class SourceTree extends FacadeTree {
  /** Create a SourceTree.
   *
   ...
   *
   * @param {string} relativePath The relative path to the directory to be scanned.
   ...
   */
  _ensureDirectoryScanned(relativePath) {
    const normalizedPath = shared.normalizeRelativePath(relativePath);
    ...
    this._entries = shared.mergeEntries(this._entries, entries, false);
   ...
   *
   * @param {string} relativePath The relative path to the subtree to be scanned.
   */
  _ensureSubtreeScanned(relativePath) {
    const normalizedPath = shared.normalizeRelativePath(relativePath);
    let entries;
    ...
    if (this._scannedDirectories.has(normalizedPath)) {
      ...
      if (index === -1) {
        ...
        while (entry.relativePath.startsWith(normalizedPathWithSeparator)) {
          if (entry.relativePath.replace(normalizedPathWithSeparator, '').indexOf('/') === -1) {
            entries.push(entry);
          }
  ...
  _findByRelativePath(relativePath) {
    ...
    if (normalizedPath !== '') {
      this._ensureDirectoryScanned(path.dirname(normalizedPath));
    }

    return super._findByRelativePath(relativePath);
  }
  ...
  _getEntriesForDirectory(normalizedPath) {
    ...
    if (entries) {
      return entries.map(entry => {
        ...
        return Entry.clone(entry, entry.relativePath.replace(shared.ensureSeparator(normalizedPath), ''));
      });
    }

    // #_ensureDirectoryScanned returns nothing if the directory has already
    // been scanned.
    return super._getEntriesForDirectory(normalizedPath);
  }
  ...
  existsSync(relativePath) {
    ...
    if (normalizedPath !== '' && !this._scannedDirectories.has(path.dirname(normalizedPath))) {
      return fs.existsSync(this._resolvePath(relativePath));
    }

    return super.existsSync(relativePath);
  }
}
 ...
 */
class WritableTree extends FacadeTree {
  ...
  /** {@see FacadeTree#_findByRelativePath} */
  _findByRelativePath(relativePath, options) {
    const normalizedPath = shared.normalizeRelativePath(relativePath);
    ...
    const currentPath = normalizedPath.split(path.sep);
    let index = shared.searchByRelativePath(this._entries, currentPath.join(path.sep));

    while (index === -1 && currentPath.length) {
      currentPath.pop();
      index = shared.searchByRelativePath(this._entries, currentPath.join(path.sep))
    }
    ...
    if (entry.relativePath === normalizedPath) {
      if (resolveSymlinks && entry.isSymbolicLink()) {
    ...
    if (entry.isSymbolicLink() && entry.isDirectory()) {
      ...
      if (normalizedPath.startsWith(relativePathWithSeparator)) {
        return entry._symlink.tree._findByRelativePath(normalizedPath.replace(relativePathWithSeparator, ''), options);
      }
    }
  ...
  _getEntriesForDirectory(normalizedPath) {
    ...
    if (normalizedPath === '') {
      return super._getEntriesForDirectory('');
    }
    ...
    if (entry.isSymbolicLink() && entry.isDirectory()) {
      ...
      if (normalizedPath.startsWith(relativePathWithSeparator)) {
        ...
        return entry._symlink.tree._getEntriesForDirectory(normalizedPath.replace(relativePathWithSeparator, ''));
      }
    }
   ...
   * @param {string=} relativePath The path being modified.
   * @param {Object=} options Additional options.
   * @param {boolean=false} options.allowRoot Whether modifications to the root should be allowed.
   */
  _throwOnCommonErrors(operation, relativePath, options) {
    ...
    if (relativePath !== undefined) {
      ...
      if (!allowRoot || normalizedPath !== '') {
        ...
        if (operation !== 'mkdirp') {
          const parentResult = this._findByRelativePath(path.dirname(normalizedPath));
          ...
          if (this !== parentResult.tree) {
            throw new Error(`Cannot ${operation} across symlinks, '${relativePath}'`);
          }
        }
      }
    }
  ...
  _track(operation_, entry) {
    ...
    this._changes.push([operation, entry.relativePath, entry]);
  }

  /** Removes a matching change from tracking.
   *
   * Only removes a single change; by the rules used in _track, multiple
   * matching changes cannot exist and it saves us a bit of time.
   *
  ...
  _untrack(operation, normalizedPath) {
    for (let i = 0; i < this._changes.length; i++) {
      ...
      if (change[0] === operation && change[1] === normalizedPath) {
  ...
  emptySync(relativePath) {
    this._throwOnCommonErrors('empty', relativePath, { allowRoot: true });
  ...
  mkdirSync(relativePath) {
    this._throwOnCommonErrors('mkdir', relativePath);
  ...
  mkdirpSync(relativePath) {
    this._throwOnCommonErrors('mkdirp', relativePath);
    ...
    const tokens = shared.normalizeRelativePath(relativePath).split(path.sep);
   ...
   */
  readFileSync(relativePath, encoding) {
    const result = this._findByRelativePath(relativePath, { resolveSymlinks: false });
    ...
    return super.readFileSync(relativePath, encoding);
  }
  ...
  readdirSync(relativePath) {
    ...
    return super.readdirSync(relativePath);
  }
  ...
  removeSync(relativePath) {
    this._throwOnCommonErrors('remove', relativePath);
  ...
  rmdirSync(relativePath) {
    this._throwOnCommonErrors('rmdir', relativePath);
  ...
  statSync(relativePath) {
    ...
    return super.statSync(relativePath);
  }
  ...
  symlinkSync(target, relativePath) {
    this._throwOnCommonErrors('symlink', relativePath);
    ...
    symlinkOrCopy.sync(target, this._resolvePath(relativePath));

    const entry = new Entry(shared.normalizeRelativePath(relativePath), undefined, Date.now(), Entry.FILE_MODE);
  ...
  symlinkToFacadeSync(targetFacade, targetRelativePath, localRelativePath) {
    ...
    this._throwOnCommonErrors('symlinkToFacade', localNormalizedPath, { allowRoot: true });
  ...
  unlinkSync(relativePath) {
    this._throwOnCommonErrors('unlink', relativePath);
    ...
    fs.unlinkSync(this._resolvePath(relativePath));
  ...
  writeFileSync(relativePath, content, options) {
    this._throwOnCommonErrors('writeFile', relativePath);
